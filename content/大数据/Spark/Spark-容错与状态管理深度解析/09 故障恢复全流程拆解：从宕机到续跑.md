...
date: 2026-02-28
tags: [Driver重启, Executor崩溃, Spark, Structured Streaming, YARN, 全流程, 容错, 故障恢复]
aliases: []
---
...
> [!note] 思考题
> 1. Executor 宕机后，Driver 通过心跳超时检测到失败，并将该 Executor 上的所有 Task 重新调度。但心跳超时时间（`spark.executor.heartbeatInterval` 和 `spark.network.timeout`）的设置存在权衡：超时时间越短，故障检测越快，但误判率越高（短暂的 GC 暂停可能触发虚假超时）。如何根据 GC 暂停时间的 P99 来合理设置心跳超时阈值？
