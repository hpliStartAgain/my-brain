...
```mermaid
graph TD
    A["应用启动\n申请 initialExecutors 个 Executor"] --> B["Executor 运行 Task"]
    B --> C{"有 pending Task?"}
    C -- "是，等待 > backlogTimeout" --> D["扩容：申请更多 Executor\n(指数增长策略)"]
    D --> B
    C -- "否" --> E{"Executor 空闲时间 > idleTimeout?"}
    E -- "否" --> B
    E -- "是，且无 Cache Block" --> F["缩容：释放空闲 Executor"]
    F --> B
    E -- "是，但有 Cache Block" --> G["保留 Executor\n(cachedExecutorIdleTimeout 保护)"]
    G --> B
