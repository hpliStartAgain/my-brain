...
date: 2026-03-03
tags: [jemalloc, LFU, LRU, maxmemory, Redis, 中间件, 内存管理, 定期删除, 惰性删除, 淘汰策略, 过期删除]
aliases: []
---
...
> [!note] 思考题
> 1. 全量同步期间主节点 fork 生成 RDB + 积累增量命令到 repl_backlog。如果 RDB 传输需要 80 秒（10GB / 1Gbps），期间的写入速率为 10MB/s——repl_backlog 需要至少 800MB。默认 `repl-backlog-size` 只有 1MB——远远不够。在什么数据量和写入速率下你需要调整这个参数？
> 2. PSYNC 2.0 支持部分重同步——从节点断连重连后通过 repl_backlog 增量同步。如果断连时间超过 repl_backlog 的覆盖窗口（`repl-backlog-size / write_rate`），仍需全量同步。在网络不稳定环境中，你如何计算所需的 repl_backlog_size 来覆盖 99% 的断连场景？
> 3. `WAIT numreplicas timeout` 同步等待指定数量的从节点确认。这提供了'半同步复制'能力——但 `WAIT` 阻塞客户端。在金融交易场景中使用 `WAIT 1 1000`（等待至少 1 个从节点确认，超时 1 秒）——如果超时未达到 numreplicas，返回实际确认数但不回滚写入。这种'尽力而为'的语义是否满足金融场景的需求？
