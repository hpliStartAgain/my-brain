...
date: 2026-03-02
tags: [B+Tree, InnoDB, MySQL, 性能优化, 索引]
aliases: [MySQL索引设计]
---
...
> [!note] 思考题
> 1. 联合索引遵循最左前缀原则——`INDEX(a, b, c)` 可以加速 `WHERE a=1`、`WHERE a=1 AND b=2`、`WHERE a=1 AND b=2 AND c=3` 的查询，但不能加速 `WHERE b=2` 或 `WHERE c=3`。在一个有多种查询模式的应用中，如何设计联合索引以覆盖尽可能多的查询？多个单列索引和一个联合索引在什么场景下性能不同？
