...
```mermaid
flowchart LR
    A([告警触发]) --> B[拓扑生成</br>Trace链路 + RPC拓扑]
    B --> C[数据采集</br>指标/告警/变更]
    C --> D[异常检测</br>SR-CNN + 孤立森林]
    D --> E[异常拓扑提取</br>递归裁剪正常节点]
    E --> F[根因分析</br>RCSF + 专家规则重排序]
    F --> G([诊断报告</br>1分钟内推送])
...
```mermaid
graph BT
    P0["基础查询</br>自然语言 → SQL / PromQL"]
    P1["拓扑感知</br>实体关联关系 · 关联分析 · 资源盘点"]
    P2["深度洞察</br>异常检测 · 趋势预测 · 模式分类"]
    P3["辅助决策</br>根因定位 · 变更分析 · 决策建议"]
...
flowchart TD
    ...
    subgraph B["第一阶段：传统算法粗筛（秒级，低成本）"]
        B1["时序异常检测</br>Isolation Forest / Prophet"]
        B2["拓扑图算法</br>随机游走 / PageRank"]
        B3["多维下钻</br>Squeeze"]
...
flowchart TD
    ...
    B["日志模板 + 频率统计"]
    C["异常模板列表（~20 条）"]
    D["RAG 检索上下文</br>异常模板 · 历史故障 · 关键指标 · 服务依赖"]
    E["LLM 推理</br>CoT 约束推理路径"]
    F["根因报告</br>时间顺序 · 因果链 · 修复建议"]

    A -- "Drain 模板化" --> B
    B -- "极简异常检测</br>5min 窗口比较" --> C
    C -- "向量化 + 元信息" --> D
    D --> E --> F
...
```mermaid
graph TD
    L0["Level 0</br>手工运维</br>全人工"] --> L1["Level 1</br>自动化运维</br>脚本/规则"]
    L1 --> L2["Level 2</br>辅助智能</br>AiOps 建议</br>人工决策"]
    L2 --> L3["Level 3</br>人机协同</br>AI 执行低风险</br>人类监督"]
    L3 --> L4["Level 4 ★</br>自主运维</br>AI 自主处置</br>大多数故障"]
    L4 -.-> L5["Level 5</br>预防性运维</br>未来终局"]
...
7. **大数据集群 AiOps 有自己的特殊性**——没有 HTTP Trace，需要用组件依赖图（HDFS→YARN→HiveServer/Spark）+变更记录+Ambari 事件来替代微服务的调用链。参见《大数据集群 AiOps 可行性分析报告》。
