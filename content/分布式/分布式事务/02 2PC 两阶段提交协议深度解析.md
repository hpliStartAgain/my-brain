...
> 2. XA 协议是 2PC 的标准实现——MySQL、PostgreSQL、Oracle 都支持 XA 事务。`XA START`、`XA END`、`XA PREPARE`、`XA COMMIT` 四个步骤。XA 事务在 Prepare 后持有锁直到 Commit——锁持有时间包括网络往返延迟。在高并发场景中，XA 的锁等待如何影响吞吐量？
