...
> [!note] 思考题
> 1. RocketMQ 的事务消息：1) 发送 Half 消息（对消费者不可见）→ 2) 执行本地事务 → 3) 根据本地事务结果提交或回滚 Half 消息。如果步骤 3 失败（如网络断开），RocketMQ 通过'消息回查'（CheckLocalTransaction）询问生产者本地事务状态。消息回查最多重试多少次？超过重试次数后如何处理？
> 2. 事务消息保证了'要么消息和本地事务都成功，要么都失败'。但消费端仍然可能失败——消费端的幂等性如何保证？在什么场景下消费端的处理必须与本地事务消息的语义一致？
> 3. Kafka 的事务消息（`transactional.id` + `initTransactions` + `commitTransaction`）也能实现类似功能——但 Kafka 事务更偏向'读取-处理-写入'的流处理场景。与 RocketMQ 的事务消息相比，Kafka 事务在'业务事务+消息'场景中的适用性如何？
