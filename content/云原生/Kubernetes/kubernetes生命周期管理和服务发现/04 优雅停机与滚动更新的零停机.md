...
date: 2026-03-04
tags: [Endpoint, Kubernetes, preStop, SIGTERM, terminationGracePeriodSeconds, 云原生, 优雅停机, 滚动更新, 零停机]
aliases: []
---
...
> [!note] 思考题
> 1. CoreDNS 是 Kubernetes 默认的 DNS 服务器——解析 Service 名称为 ClusterIP。Pod 的 `/etc/resolv.conf` 中 `nameserver` 指向 CoreDNS 的 Service IP。`ndots:5`（默认）意味着查询少于 5 个点的域名时，会先追加搜索域后缀尝试解析。查询 `www.google.com`（3 个点<5）会先尝试 `www.google.com.default.svc.cluster.local`——产生 4 次无效查询。你如何通过设置 `ndots:2` 或使用 FQDN（末尾加点 `www.google.com.`）来减少无效 DNS 查询？
