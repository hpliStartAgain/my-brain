...
> [!note] 思考题
> 1. NFS v3 无状态、v4 有状态。v4 的有状态模型支持文件锁和委托（delegation）。v4 的 delegation 允许客户端缓存打开的文件而无需每次操作都与服务端通信——这对性能有什么提升？delegation 在什么情况下会被服务端收回（recall）？
> 2. NFS 的 `close-to-open` 一致性语义不保证文件打开期间的一致性。SQLite on NFS 可能出现数据库损坏——因为 SQLite 依赖 `fcntl` 文件锁做并发控制，而 NFS 的文件锁在网络分区时可能丢失。除了 SQLite，还有哪些应用不适合在 NFS 上运行？
> 3. NFS 的 `async` 挂载选项提高性能但增加数据丢失风险。在开发环境（数据不重要、性能优先）中使用 `async` 是合理的。但在什么场景下 `async` 可能导致数据损坏而非仅丢失（提示：考虑 rename 操作的顺序性）？
