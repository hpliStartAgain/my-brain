...
> [!note] 思考题
> 1. 一个 Java 容器（limit 4Gi、-Xmx2g）被 OOM Kill，堆使用只有 1.2GB。列出至少四个可能的堆外内存来源，以及各自的观测方法。
> 2. CPU steal time 高说明什么？为什么"加 CPU 配额"可能无助于解决问题？正确的应对路径是什么？
> 3. 为什么 GPU 推理的瓶颈常常在 PCIe 而不是 GPU 算力？从"数据搬运 vs 计算"的角度分析，并给出至少两种优化手段。
> 4. Project Valhalla 的值类型如何同时改善内存密度和缓存利用率？为什么说它对 GC 压力的改善是"釜底抽薪"式的？
