...
TLAB 的性能意义值得量化理解：无 TLAB 时，每次对象分配需要 CAS 竞争 Eden 的分配指针——高并发下 CAS 失败重试的开销显著；有 TLAB 时，分配退化为"指针移动 + 对象头初始化"，约 10ns，与 C 的 malloc 相当甚至更快（无空闲链表查找）。**Java 对象分配比 C 快**——这个反直觉结论的机制就在 TLAB：C 的 malloc 要在空闲链表中找合适大小的块，Java 的 TLAB 分配是纯指针碰撞。这个机制也解释了为什么 Java 敢于"大量分配短命对象"——分配便宜 + 回收集中（Minor GC），是分代设计的完整闭环。

TLAB 的观测有一个实用技巧：**TLAB 外分配是"分配热点"的信号**。JFR 的 `jdk.ObjectAllocationOutsideTLAB` 事件记录绕过快速路径的分配——它们要么是大对象（超过 TLAB 剩余空间），要么是分配速率异常高（TLAB 频繁 refill）。这个事件与分配剖析（第 04 篇 async-profiler 的 alloc 模式）结合，能定位"分配压力"的代码源头——这是 GC 调优"应用优先"原则（第 9 章）的观测入口。
...
G1 的参数很多，但调优的杠杆效应差异巨大。按"收益/风险比"排序的调优顺序：

**第一杠杆：`MaxGCPauseMillis`（软目标）**。它是 G1 自适应调整的总开关——设 200ms，G1 会自动调整年轻代大小、混合回收节奏来逼近目标。**大多数 G1 调优问题只需要动这一个参数**。调小 → 停顿短但 GC 频繁；调大 → 停顿长但频率低。默认 200ms 适合多数在线服务。

**第二杠杆：堆大小与 Region 大小**。堆大小决定"GC 频率 vs 单次停顿"的基本盘；Region 大小（1-32MB 自动选择）影响 Humongous 阈值（Region 的 50%）——应用有 10MB 大对象时，Region 至少要 32MB 才能让它不成为 Humongous。

**第三杠杆：IHOP 与并发标记**。自适应 IHOP（JDK 9+ 默认）在稳态负载下表现良好，突发分配场景才需要固定值（3.3 节的避坑）。

**不建议碰的参数**：`G1RSetRegionEntries`、`G1ConcRefinementThreads` 等"内部机制"参数——它们在 JDK 版本间的语义会变，覆盖默认值的风险远大于收益。**G1 调优的 90% 场景只需要 `MaxGCPauseMillis` + 堆大小 + `G1ReservePercent`**，其余参数留给自适应机制。

这个"少调优"立场有一个重要的反面：**调优的验证必须做**。即使只改一个参数，也要在预发布环境用生产级负载模型验证（第 12 篇），并保留回滚路径——GC 参数的"看似无害"调整（譬如 Region 大小）可能引发 Humongous 阈值变化等连锁反应。
...
ZGC 的亚毫秒停顿很诱人，但它不是普适升级。四个适用性判断维度：

**堆大小**：ZGC 的优势在"大堆"场景——堆越大，传统 GC 的停顿越长，ZGC 的"停顿与堆大小无关"优势越大。8GB 以下的小堆，G1 的停顿本来就只有几毫秒，ZGC 的屏障开销反而得不偿失。**经验分界线在 16-32GB**。

**吞吐敏感度**：ZGC 的屏障开销让吞吐下降 5-15%。吞吐优先的批处理（数据分析、离线计算）用 G1 或 Parallel 更划算——它们的 STW 停顿对"总完成时间"影响有限。

**分配速率**：ZGC 的并发标记/转移需要跟上分配速率，分配速率过高时会出现 Allocation Stall（分配停滞）。高分配速率（> 3GB/s）的应用要实测 ZGC 的表现，不能只看宣传数字。

**JDK 版本**：分代 ZGC（JDK 21+）解决了原始 ZGC 的吞吐短板，是当前 ZGC 的推荐形态。JDK 17 的非分代 ZGC 对高分配速率负载的表现明显弱于分代版。

一个选型的快速判断：**停顿 SLO < 10ms 且堆 > 16GB → ZGC；吞吐优先或堆小 → G1；批处理且不在乎停顿 → Parallel**。这个判断覆盖 80% 的场景，剩下 20%（譬如超大堆 + 极致吞吐）需要实测——第 12 篇的基准测试方法论是选型验证的工具。
...
屏障开销的机制值得拆开看，因为它解释了"为什么并发 GC 慢"的微观原因。以 G1 的写屏障为例，每次引用赋值（`obj.field = other`）在编译后的代码里变成三步：正常赋值 + 检查是否跨 Region 引用 + 跨 Region 时记录到写缓冲。这个"检查 + 记录"是几条额外指令——单次纳秒级，但应用每秒执行数亿次引用赋值，累积就是 5-10% 的吞吐。ZGC 的读屏障更重：每次读引用都要检查染色位，可能触发"自愈"（查转发表、更新引用）——这就是 ZGC 吞吐低于 G1 的机制根源。
...
SATB 与增量更新（Incremental Update，CMS 的方案）是并发标记的两大流派，它们的差异影响浮动垃圾量：SATB 记录"被覆盖的旧引用"——标记开始时存活的对象即使立刻死亡也会被当作活的（浮动垃圾多，但标记逻辑简单）；增量更新记录"新引用的加入"——需要重新扫描，但浮动垃圾少。G1 选 SATB 是用浮动垃圾换标记吞吐，ZGC 的染色指针方案同理。**理解这个权衡，就能理解为什么并发 GC 的堆占用水位线（IHOP）要留出浮动垃圾的空间**——IHOP 调优（3.3 节）的余量需求正来自这里。
...
这段日志还有一层"效率分析"的价值，值得展开。**回收效率 = 回收量 / 停顿时间**：本次回收 128MB 用了 49.6ms，效率约 2.6MB/ms。如果另一次 GC 回收 20MB 用了 45ms（效率 0.4MB/ms），说明这次 GC"花同样的时间却几乎没回收东西"——典型的"活对象太多"（LDS 过大）信号。**回收效率的持续下降是堆健康度的领先指标**：它先于 Full GC 出现，给调优留出窗口。GC 日志分析工具（GCEasy、GCViewer）会输出这个比值，手工分析时用"回收量/停顿"快速心算即可。

另一个分析维度是**各阶段的耗时占比**：Evacuate（复制存活对象）占 45.2ms 是主体，Pre/Post 阶段合计 3.2ms 是固定开销。如果 Pre/Post 占比异常高（譬如各占 20ms），说明问题不在"复制"而在"GC 的组织开销"（根扫描、RSet 处理）——优化方向完全不同。**先分阶段再谈优化**，是 GC 日志分析的基本纪律。
...
| 回收效率下降 | 停顿不变但回收量减少 | LDS 持续增长（缓存膨胀/泄漏） | 堆直方图 + 引用链分析 |

这张病理表的用法是"从症状到根因"的映射，但使用它有一个前置纪律：**先确认症状的"时间模式"**。同样的"Mixed GC 停顿长"，渐进恶化（数天）指向 LDS 缓慢增长（缓存泄漏），突变（发布后立刻）指向负载变化（新版本引入大对象）——时间模式决定了是查泄漏还是查变更。第 14 篇的"变更窗口检查"在这里同样适用：GC 行为突变的第一排查动作是"最近 24 小时改了什么"。
...
流程中每一步的"完成标志"值得明确，这是流程可执行的关键：

- **基线测量**的完成标志：有一份覆盖高峰时段的 gc.log + JFR 录制，能回答"分配速率、LDS、晋升速率、GC 频率/停顿分布"五个数字。
- **工作负载分析**的完成标志：能回答"对象生命周期画像"——多少比例朝生夕灭、中等生命周期对象占多少、大对象比例。
- **选型**的完成标志：按 7.2 决策树走完，明确候选 GC 与预期权衡。
- **基准测试**的完成标志：候选 GC 在"生产级负载模型"（第 12 篇）下对比过停顿分布与吞吐，且差异超出置信区间。
- **增量调优**的完成标志：每次只动一个参数，前后各采集 24 小时数据对比——同时改多个参数的"调优"无法归因，等于没调。
...
## 第 9 章 案例复盘：一次"GC 停顿超标"的完整调优

用一个生产案例串联本章的方法论。这个案例的典型性在于：根因不在 GC 参数，而在应用的对象生命周期模式——"先修应用，再调 GC"原则的完整演绎。

**现象**：一个风控规则引擎服务，16GB 堆、G1 GC、`MaxGCPauseMillis=100`。上线新规则集后，Mixed GC 停顿从 80ms 恶化到 400ms+，P99 延迟超标。团队的第一反应是"调 GC 参数"。

**第一轮：GC 日志分析（定位阶段）。** `-Xlog:gc*=debug` 的关键信号：
- Mixed GC 的 Evacuate 阶段从 60ms 涨到 350ms——瓶颈在"复制存活对象"
- 回收效率从 3MB/ms 跌到 0.5MB/ms——**同样的复制时间，回收量只有原来的 1/6**
- `gc+humongous` 日志显示新规则集引入大量 2-8MB 的规则编译产物（Humongous 对象）

**根因**：新规则引擎把每条规则编译为一个大对象图（解释器树），规则集从 200 条涨到 1200 条——老年代活对象（LDS）从 4GB 涨到 11GB。Mixed GC 每次要复制/扫描的活对象暴增，停顿自然恶化。**这是负载变化的问题，不是 GC 配置的问题**——16GB 堆装 11GB 活对象，本就接近极限。

**第二轮：修复方案评估（权衡取舍）。** 三个候选方案：
1. **扩堆到 32GB**：治标，且 32GB 跨 NUMA 节点（第 06 篇的陷阱）、指针压缩失效（第 05 篇的悬崖）——副作用大。
2. **规则对象瘦身**：把解释器树改为扁平化字节码数组——活对象体积降 60%，LDS 回落到 5GB。改动中等，收益最大。
3. **换 ZGC**：停顿问题消失，但吞吐降 10-15%、堆开销增加——且"活对象太多"的本质没变，只是把停顿转嫁为吞吐。

团队选了方案 2（对象瘦身）+ 方案 3 的评估并行：先做瘦身（治本），同时压测 ZGC 作为后备。瘦身上线后 LDS 回到 5GB，Mixed GC 停顿回到 90ms，问题解决——ZGC 无需引入。

**复盘的方法论启示**：

1. **回收效率是"负载 vs 容量"的体温计**。GC 停顿变长时，先算"回收量/停顿时间"——效率下降说明活对象变多，问题在应用的对象生命周期，不在 GC 参数。调参数（扩堆、换 GC）是掩盖症状，对象瘦身才是治疗。
2. **Humongous 对象是 G1 的放大器**。大对象直接进老年代、连续 Region 分配、回收代价高——规则引擎这类"动态编译大对象图"的场景要特别监控 `gc+humongous` 日志。
3. **GC 选型是最后手段**。方案 3（换 ZGC）在这个案例里是"用吞吐换停顿"的权衡——它有效但昂贵。先穷尽应用层优化（对象瘦身、缓存策略、分配速率），再考虑 GC 层的换血。这与第 01 篇"收益递减"原则一致：应用层的修复收益最大、代价最小。

> [!note] 设计哲学：GC 调优的"应用优先"原则
> 这个案例是本章反复出现的主题的终章：**GC 是内存管理的"结果"，不是"原因"**。GC 停顿长，直接原因可能是"活对象多""分配速率高""Region 碎片"，但这些的直接原因几乎都在应用层——缓存无界、对象过大、生命周期错配。成熟的 GC 调优者的第一反应不是"调哪个参数"，而是"应用在制造什么样的内存流量"。GC 日志、JFR 分配采样（第 04 篇）、堆直方图是回答这个问题的工具链。参数调优（第 7.3 节）是最后一公里的微调，不是第一反应。
...
1. Monica Beckwith, *JVM Performance Engineering*, O'Reilly, 2024. 第 6 章 "OpenJDK 高级内存管理与垃圾回收"
2. Brendan Gregg, *Systems Performance*, 2nd Edition, 2020. GC 相关章节
3. Oracle 文档, G1 GC Tuning Guide, https://docs.oracle.com/en/java/javase/21/gctuning/
4. JEP 439: Generational ZGC, https://openjdk.org/jeps/439
5. JEP 377: Shenandoah: A Garbage-First Collector 的对偶实现, https://openjdk.org/jeps/404
6. Charlotte Herzeel 等, GC 日志分析工具 GCViewer, https://github.com/chewiebug/GCViewer
7. Kirk Pepperdine, GC 调优系列文章（"GC 调优的先观察后调参"原则）

---

> [!note] 思考题
> 1. 一个服务 Mixed GC 停顿从 80ms 恶化到 400ms，GC 日志显示回收效率从 3MB/ms 跌到 0.5MB/ms。这个信号说明什么？你的排查方向是调 GC 参数还是查应用？为什么？
> 2. 为什么 ZGC 的吞吐低于 G1？从读屏障的机制解释，并说明什么样的负载"值得"付出这个代价。
> 3. 弱分代假说在什么负载下不成立？举一个"中等生命周期对象占主导"的业务场景，并说明它对分代 GC 的影响。
> 4. 自适应 IHOP 在什么场景下会失效？失效的后果是什么？如何修复？
