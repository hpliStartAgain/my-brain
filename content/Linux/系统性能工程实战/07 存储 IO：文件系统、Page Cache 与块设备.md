...
> [!note] 思考题
> 1. 一个服务的应用层读延迟 P99 从 100μs 恶化到 5ms，但 `iostat` 显示磁盘 `await` 只有 0.5ms。可能的原因是什么？按"分层叠加"模型给出你的排查顺序。
> 2. 为什么数据库要用 `O_DIRECT` 绕过 Page Cache，而普通 Web 服务不应该？从"缓存所有权"的角度分析。
> 3. NVMe 相比 SATA SSD 的优势不只是 IOPS。从协议栈、队列模型、CPU 效率三个维度解释"NVMe 是架构范式跃迁"的含义。
> 4. 一次 `fsync` 的延迟下限由什么决定？为什么数据库的 commit 延迟无法通过软件优化降到 1ms 以下（机械盘场景）？
