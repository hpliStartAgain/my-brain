...
[[多处理器]]（Multiprocessor）支持允许操作系统同时使用多个 CPU 执行其任务。目前的标准是[[对称多处理]]（Symmetric Multiprocessing, SMP），其中所有 CPU 都在一个一致的主存视图（尽管通过了缓存）下平等地管理。

多处理器支持增加了调度器的复杂性，调度器现在应力求将线程负载平衡地分配给所有可用的 CPU。它还引入了[[多处理器性能问题]]，包括 CPU 之间的资源争用以及在 CPU 之间移动线程导致缓存变冷（缓存热度损失）。

内核可以通过[[处理器间中断]]（Inter-Processor Interrupts, IPIs）使 CPU 进行相互通信。这些中断用于协调多处理器操作，例如在 CPU 之间分发时钟滴答或触发 TLB 刷新。

随着物理处理器的增多，主存与处理器之间的连接（总线）可能成为瓶颈。为了解决这个问题，开发了[[非统一内存访问]]（Non-Uniform Memory Access, NUMA）架构，其中内存被划分为多个组（NUMA 节点），每个组与一组 CPU 紧密关联。虽然 CPU 仍然可以访问所有内存，但访问与其关联的本地内存速度更快。这引入了新的调度复杂性：调度器不仅应力求负载平衡，还应努力使线程保持在与其内存所在的相同 NUMA 节点上。见第 6 章“CPU”。

### 3.2.15 抢占 (Preemption)

内核[[抢占]]（Preemption）允许高优先级的线程随时中断当前正在内核态运行的低优先级线程，以执行其工作。这对于提供低延迟和确定性响应时间至关重要。

在不支持抢占的早期内核中，一个运行在内核态（例如，执行一个较长的系统调用）的线程无法被中断。高优先级的线程必须等待该系统调用完成或线程主动让出 CPU（自愿让出）。这会引入显著的调度延迟，对于需要低延迟响应的应用（如音频处理或工业控制）是不利的。

实现完全抢占的内核（Fully preemptible kernel）非常复杂，因为这意味着内核代码本身必须是可重入的，并且需要复杂的锁定机制来保护内核数据结构免受并发访问。

Linux 支持多种抢占模式。其中一种是[[自愿内核抢占]]（Voluntary kernel preemption），它通过在内核代码中频繁添加“抢占点”（显式的调度检查）来实现低延迟，而无需完全抢占的复杂性。Linux 也支持[[完全抢占]]，尤其是在应用了实时补丁（PREEMPT_RT）后。

### 3.2.16 资源管理 (Resource Management)

操作系统提供了限制进程和线程资源消耗的机制，以防止单个进程耗尽系统资源并影响其他进程。

- **进程优先级**：通过 `nice(2)` 系统调用手动调整。
- **资源限制 (Resource Limits)**：通过 `setrlimit(2)` 系统调用设置，并由 `ulimit` 命令控制。这可以限制每个进程的文件描述符数量、最大堆栈大小、最大内存消耗等。
- **控制组 (Control Groups, cgroups)**：这是 Linux 的一项功能，允许将进程组织成组，并为这些组分配资源（CPU、内存、磁盘 I/O、网络）。cgroups 是容器技术（如 Docker 和 Kubernetes）的基础。

cgroups 最初有几个不同的层次结构（v1），这被证明是难以管理的。Linux 后来引入了统一的层次结构，称为 [[cgroup v2]]，它简化了资源管理并提供了更好的功能，包括压力阻塞信息（Pressure Stall Information, PSI），用于衡量系统在资源压力下的表现。见第 11 章“云计算”。

### 3.2.17 可观测性 (Observability)

操作系统由内核、库和程序组成。这些程序包括用于观察系统活动和分析性能的工具，通常安装在 `/usr/bin` 和 `/usr/sbin`。系统上还可能安装第三方工具，以提供额外的可观测性。

可观测性工具，以及构建它们所依赖的操作系统组件，将在第 4 章中介绍。
...
接下来的章节讨论类 Unix 内核的实现细节，重点关注性能。作为背景，讨论了早期内核的性能特性：Unix、BSD 和 Solaris。Linux 内核将在 3.4 节“Linux”中进行更详细的讨论。

内核差异可能包括它们支持的文件系统（见第 8 章“文件系统”）、系统调用（syscall）接口、网络栈架构、实时支持，以及 CPU、磁盘 I/O 和网络的调度算法。

表 3.3 显示了 Linux 和其他内核版本的对比，系统调用计数基于操作系统帮助手册第 2 部分中的条目数量。这是一个粗略的比较，但足以看出一些差异。

**表 3.3 带有记录的系统调用计数的内核版本**
...
这些只是带有文档的系统调用；内核通常还提供更多供操作系统软件私用的系统调用。

> 最初 UNIX 只有 20 个系统调用，而今天的 Linux——它是直接后裔——拥有超过一千个……我只是担心不断增长的事物的复杂性和规模。
> —— Ken Thompson，ACM 图灵奖百周年庆典，2012 年

Linux 的复杂性正在增长，并通过添加新的系统调用或其他内核接口将这种复杂性暴露给用户层。额外的复杂性使得学习、编程和调试更加耗时。

### 3.3.1 Unix

Unix 由 Ken Thompson、Dennis Ritchie 和其他人在 1969 年及随后的几年里在 AT&T 贝尔实验室开发。其确切起源在《The UNIX Time-Sharing System》[Ritchie 74] 中有描述：

> 当我们中的一个人（Thompson）对可用的计算机设施感到不满时，发现了一台很少使用的 PDP-7，并着手创建一个更友好的环境，于是编写了第一个版本。

UNIX 的开发人员之前曾参与过 Multics（多路信息与计算服务）操作系统的开发。UNIX 被开发为一个轻量级的多任务操作系统和内核，最初命名为 UNICS（单路信息与计算服务），是对 Multics 的双关。摘自《UNIX Implementation》[Thompson 78]：

> 内核是唯一的不能由用户根据自己的喜好进行替换的 UNIX 代码。出于这个原因，内核应该尽可能少地做出真正的决定。这并不意味着允许用户有一百万种选择来做同一件事。相反，它意味着只允许用一种方式做一件事，但让这种方式成为所有可能提供的选项的最小公分母。

虽然内核很小，但它确实提供了一些高性能特性。进程具有调度优先级，减少了高优先级工作的运行队列延迟。磁盘 I/O 以大（512 字节）块的形式执行以提高效率，并缓存到内存中每个设备的[[缓冲区缓存]]（Buffer cache）中。空闲进程可以被[[交换]]（Swapped）到存储设备中，从而允许更繁忙的进程在主存中运行。而且系统当然是多任务的——允许多个进程并发运行，提高了作业吞吐量。

为了支持网络、多个文件系统、分页以及我们现在认为标准的其他特性，内核不得不增长。随着包括 BSD、SunOS (Solaris) 以及后来的 Linux 在内的多种衍生版本的出现，内核性能变得具有竞争力，这促使添加了更多的特性和代码。

### 3.3.2 BSD

Berkeley 软件发布（BSD）操作系统始于加州大学伯克利分校对 Unix 第 6 版的增强，并于 1978 年首次发布。由于原始 Unix 代码需要 AT&T 软件许可证，到 1990 年代初，这些 Unix 代码在 BSD 协议下被重写，允许包括 FreeBSD 在内的自由发布。

主要的 BSD 内核开发，特别是与性能相关的开发，包括：

- **分页虚拟内存**：BSD 将分页虚拟内存引入了 Unix：不是通过交换整个进程来释放主存，而是可以移动（分页）较小的最近最少使用（LRU）的内存块。见第 7 章“内存”7.2.2 节“分页”。
- **请求分页**：这会将物理内存到虚拟内存的映射推迟到首次写入时，避免了对可能永远不会使用的页面的早期且有时是不必要的性能和内存开销。请求分页是由 BSD 引入 Unix 的。见第 7 章“内存”7.2.2 节“分页”。
- **FFS**：Berkeley 快速文件系统（FFS）将磁盘分配划分为柱面组，大大减少了碎片并提高了旋转磁盘的性能，并支持更大的磁盘和其他增强功能。FFS 成为许多其他文件系统的基础，包括 UFS。见第 8 章“文件系统”8.4.5 节“文件系统类型”。
- **TCP/IP 网络栈**：BSD 为 Unix 开发了第一个高性能 TCP/IP 网络栈，包含在 4.2BSD（1983 年）中。BSD 仍以其高性能的网络栈而闻名。
- **套接字 (Sockets)**：Berkeley 套接字是连接端点的 API。包含在 4.2BSD 中，它们已成为网络的标准。见第 10 章“网络”。
- **Jails**：轻量级操作系统级虚拟化，允许多个访客共享一个内核。Jails 首次发布于 FreeBSD 4.0。
- **内核 TLS**：由于传输层安全（TLS）现在在互联网上普遍使用，内核 TLS 将大部分 TLS 处理移至内核，提高了性能[^14] [Stewart 15]。

虽然不如 Linux 流行，但 BSD 被用于一些性能关键型环境，包括 Netflix 的内容分发网络（CDN），以及来自 NetApp、Isilon 等公司的文件服务器。Netflix 在 2019 年总结了其 CDN 上的 FreeBSD 性能 [Looney 19]：

> “使用 FreeBSD 和通用部件，我们在 16 核 2.6-GHz CPU 上，以约 55% 的 CPU 利用率实现了 90 Gb/s 的 TLS 加密连接服务。”

关于 FreeBSD 的内核实现，有一本优秀的参考书，由出版此书的同一家出版社出版：《The Design and Implementation of the FreeBSD Operating System, 2nd Edition》[McKusick 15]。

### 3.3.3 Solaris

Solaris 是由 Sun Microsystems 在 1982 年创建的派生自 Unix 和 BSD 的内核及操作系统。它最初被命名为 SunOS，并针对 Sun 工作站进行了优化。到 1980 年代后期，AT&T 开发了一个新的 Unix 标准：基于 SVR3、SunOS、BSD 和 Xenix 技术的 Unix System V Release 4 (SVR4)。Sun 基于 SVR4 创建了一个新内核，并以 Solaris 的名称重新命名了该操作系统。

主要的 Solaris 内核开发，特别是与性能相关的开发，包括：

- **VFS**：[[虚拟文件系统]]（VFS）是一种抽象和接口，允许各种文件系统轻松共存。Sun 最初创建它是为了让 NFS 和 UFS 可以共存。VFS 在第 8 章“文件系统”中涵盖。
- **全抢占式内核**：这为高优先级工作（包括实时工作）提供了低延迟。
- **多处理器支持**：在 1990 年代初期，Sun 在多处理器操作系统支持方面投入巨资，开发了非对称和对称多处理（ASMP 和 SMP）的内核支持 [Mauro 01]。
- **Slab 分配器**：取代了 SVR4 的伙伴分配器（Buddy allocator），内核 slab 内存分配器通过每个 CPU 的预分配缓冲区缓存提供了更好的性能，这些缓冲区可以被快速重用。这种分配器类型及其衍生产品已成为包括 Linux 在内的内核的标准。
- **DTrace**：一个静态和动态追踪框架及工具，提供对整个软件栈几乎无限的可观测性，支持实时和生产环境。Linux 拥有用于此类可观测性的 BPF 和 bpftrace。
- **Zones**：一种基于操作系统的虚拟化技术，用于创建共享一个内核的操作系统实例，类似于早期的 FreeBSD jails 技术。操作系统虚拟化现在作为 Linux 容器被广泛使用。见第 11 章“云计算”。
- **ZFS**：一个具有企业级特性和性能的文件系统。它现在可用于其他操作系统，包括 Linux。见第 8 章“文件系统”。

Oracle 在 2010 年收购了 Sun Microsystems，Solaris 现在被称为 Oracle Solaris。Solaris 在本书的第一版中有更详细的介绍。

## 3.4 Linux

Linux 由 Linus Torvalds 于 1991 年创建，作为用于 Intel 个人电脑的自由操作系统。他在 Usenet 帖子中宣布了该项目：

> 我正在为 386(486) AT 兼容机做一个（自由的）操作系统（只是一个爱好，不会像 gnu 那样大而专业）。这个项目从 4 月就开始酝酿了，现在开始准备好了。我希望能得到关于人们在 minix 中喜欢/不喜欢什么的反馈，因为我的系统在某些方面与之相似（除了其他方面，文件系统的物理布局相同（由于实际原因））。

这指的是 MINIX 操作系统，它当时被开发为用于小型计算机的自由且微型的 Unix 版本。BSD 当时也旨在提供一个自由的 Unix 版本，尽管当时它正面临法律纠纷。

Linux 内核是在借鉴了许多前辈的通用思想的基础上开发的，包括：

- **Unix (和 Multics)**：操作系统分层、系统调用、多任务、进程、进程优先级、虚拟内存、全局文件系统、文件系统权限、设备节点、缓冲区缓存
- **BSD**：分页虚拟内存、请求分页、快速文件系统 (FFS)、TCP/IP 网络栈、套接字
- **Solaris**：VFS、NFS、页缓存、统一页缓存、slab 分配器
- **Plan 9**：资源分叉 (rfork)，用于在进程和线程（任务）之间创建不同层级的共享

Linux 现在广泛应用于服务器、云实例和包括移动电话在内的嵌入式设备。

### 3.4.1 Linux 内核开发 (Linux Kernel Developments)

Linux 内核的开发，特别是那些与性能相关的开发，包括以下内容（其中许多描述包含了它们首次引入时的 Linux 内核版本）：

- **CPU 调度类**：已开发了各种先进的 CPU 调度算法，包括调度域 (2.6.7) 以针对非统一内存访问 (NUMA) 做出更好的决策。见第 6 章“CPU”。
- **I/O 调度类**：已开发了不同的块 I/O 调度算法，包括 deadline (2.5.39)、anticipatory (2.5.75) 和完全公平队列 (CFQ) (2.6.6)。这些在 Linux 5.0 之前的内核中可用，5.0 版本为了支持仅有的较新的多队列 I/O 调度程序而删除了它们。见第 9 章“磁盘”。
- **TCP 拥塞算法**：Linux 允许配置不同的 TCP 拥塞控制算法，并在本列表后面提到的较新内核中支持 Reno、Cubic 等。另见第 10 章“网络”。
- **过量使用 (Overcommit)**：连同内存不足 (OOM) 杀手，这是一种用较少主存做更多事情的策略。见第 7 章“内存”。
- **Futex (2.5.7)**：快速用户空间互斥锁（fast user-space mutex）的缩写，用于提供高性能的用户级同步原语。
- **大页 (Huge pages) (2.5.36)**：提供了由内核和内存管理单元 (MMU) 预分配大内存页的支持。见第 7 章“内存”。
- **OProfile (2.5.43)**：一个用于研究内核和应用程序的 CPU 使用情况及其他事件的系统剖析器。
- **RCU (2.5.43)**：内核提供了一种读取-复制更新（read-copy update）同步机制，允许在进行更新的同时并发进行多次读取，提高了以读为主的数据的性能和可扩展性。
- **epoll (2.5.46)**：一个用于高效等待多个打开的文件描述符上的 I/O 的系统调用，提高了服务器应用程序的性能。
- **模块化 I/O 调度 (2.6.10)**：Linux 为调度块设备 I/O 提供了可插拔的调度算法。见第 9 章“磁盘”。
- **DebugFS (2.6.11)**：一个简单的非结构化接口，用于内核向用户层公开数据，被一些性能工具使用。
- **Cpusets (2.6.12)**：为进程提供排他性的 CPU 分组。
- **自愿内核抢占 (2.6.13)**：此过程提供低延迟调度，而无需全抢占的复杂性。
- **inotify (2.6.13)**：一个用于监控文件系统事件的框架。
- **blktrace (2.6.17)**：一个用于追踪块 I/O 事件的框架和工具（后来迁移到了 tracepoints 中）。
- **splice (2.6.17)**：一个在文件描述符和管道之间快速移动数据而无需经过用户空间的系统调用。（`sendfile(2)` 系统调用现在是 `splice(2)` 的封装，它可以高效地在文件描述符之间移动数据。）
- **延迟记账 (Delay accounting) (2.6.18)**：跟踪每个任务的延迟状态。见第 4 章“可观测性工具”。
- **IO 记账 (IO accounting) (2.6.20)**：测量每个进程的各种存储 I/O 统计信息。
- **DynTicks (2.6.21)**：动态滴答允许内核定时器中断（时钟）在空闲期间不触发，节省 CPU 资源和功耗。
- **SLUB (2.6.22)**：slab 内存分配器的一个新的且简化的版本。
- **CFS (2.6.23)**：完全公平调度器（Completely fair scheduler）。见第 6 章“CPU”。
- **cgroups (2.6.24)**：控制组允许测量和限制进程组的资源使用。
- **TCP LRO (2.6.24)**：TCP 大型接收卸载 (LRO) 允许网络驱动程序和硬件在将数据包发送到网络栈之前将其聚合为更大的尺寸。Linux 还支持发送路径的大型发送卸载 (LSO)。
- **latencytop (2.6.25)**：用于观察操作系统中延迟来源的插桩和工具。
- **Tracepoints (2.6.28)**：静态内核追踪点（又称静态探针），用于插桩内核中的逻辑执行点，供追踪工具使用（以前称为内核标记 kernel markers）。第 4 章“可观测性工具”介绍了追踪工具。
- **perf (2.6.31)**：Linux 性能事件 (perf) 是一套用于性能可观测性的工具，包括 CPU 性能计数器剖析以及静态和动态追踪。见第 6 章“CPU”了解介绍。
- **无 BKL (2.6.37)**：最终消除了大内核锁 (BKL) 性能瓶颈。
- **透明大页 (Transparent huge pages) (2.6.38)**：这是一个允许轻松使用大内存页的框架。见第 7 章“内存”。
- **KVM**：基于内核的虚拟机 (KVM) 技术是由 Qumranet 为 Linux 开发的，该公司于 2008 年被 Red Hat 收购。KVM 允许创建运行其自身内核的虚拟操作系统实例。见第 11 章“云计算”。
- **BPF JIT (3.0)**：针对伯克利包过滤器 (BPF) 的即时 (JIT) 编译器，通过将 BPF 字节码编译为原生指令来提高包过滤性能。
- **CFS 带宽控制 (3.2)**：一种支持 CPU 配额和节流的 CPU 调度算法。
- **TCP 反 bufferbloat (3.3+)**：从 Linux 3.3 开始进行了各种增强以对抗 bufferbloat 问题，包括用于数据包数据传输的字节队列限制 (BQL) (3.3)、CoDel 队列管理 (3.5)、TCP 小队列 (3.6) 以及比例积分控制器增强 (PIE) 数据包调度程序 (3.14)。
- **uprobes (3.5)**：用于用户级软件动态追踪的基础设施，被其他工具（perf, SystemTap 等）使用。
- **TCP 早期重传 (3.5)**：RFC 5827，用于减少触发快速重传所需的重复确认。
- **TFO (3.6, 3.7, 3.13)**：TCP 快速打开 (TFO) 可以通过带有 TFO cookie 的单个 SYN 包将 TCP 三次握手减少为一次，从而提高性能。它在 3.13 中成为默认设置。
- **NUMA 平衡 (3.8+)**：这增加了内核在多 NUMA 系统上自动平衡内存位置的方法，减少了 CPU 互连流量并提高了性能。
- **SO_REUSEPORT (3.9)**：一个套接字选项，允许多个监听套接字绑定到同一个端口，提高了多线程可扩展性。
- **SSD 缓存设备 (3.9)**：设备映射器 (Device mapper) 支持将 SSD 设备用作较慢旋转磁盘的缓存。
- **bcache (3.10)**：用于块接口的 SSD 缓存技术。
- **TCP TLP (3.10)**：TCP 尾部丢失探测 (TLP) 是一种通过在较短的探测超时后发送新数据或最后一个未确认段来避免代价高昂的基于定时器的重传方案，以触发更快的恢复。
- **NO_HZ_FULL (3.10, 3.12)**：也称为无计时器多任务或无滴答内核，这允许非空闲线程在没有时钟滴答的情况下运行，避免了工作负载扰动 [Corbet 13a]。
- **多队列块 I/O (3.13)**：这提供了每个 CPU 的 I/O 提交队列，而不是单个请求队列，提高了可扩展性，特别是对于高 IOPS 的 SSD 设备 [Corbet 13b]。
- **SCHED_DEADLINE (3.14)**：一个可选的调度策略，实现了最早截止时间优先 (EDF) 调度 [Linux 20b]。
- **TCP autocorking (3.14)**：这允许内核合并小的写入，减少发送的数据包。是 `TCP_CORK` `setsockopt(2)` 的自动版本。
- **MCS 锁和 qspinlocks (3.15)**：高效的内核锁，使用每 CPU 结构等技术。MCS 以原始锁发明者（Mellor-Crummey 和 Scott）的名字命名 [Mellor-Crummey 91][Corbet 14]。
- **扩展 BPF (3.18+)**：一个用于运行安全内核模式程序的内核执行环境。扩展 BPF 的大部分是在 4.x 系列中添加的。对挂载到 kprobes 的支持在 3.19 中添加，对 tracepoints 在 4.7 中添加，对软件和硬件事件在 4.9 中添加，对 cgroups 在 4.10 中添加。有界循环在 5.3 中添加，这也增加了指令限制以允许复杂的应用程序。见 3.4.4 节“扩展 BPF”。
- **Overlayfs (3.18)**：Linux 中包含的一种联合挂载文件系统。它在其他文件系统之上创建虚拟文件系统，这些虚拟文件系统也可以在不改变第一个文件系统的情况下进行修改。常用于容器。
- **DCTCP (3.18)**：数据中心 TCP (DCTCP) 拥塞控制算法，旨在提供高突发容忍度、低延迟和高吞吐量 [Borkmann 14a]。
- **DAX (4.0)**：直接访问 (DAX) 允许用户空间直接从持久内存存储设备读取，而无需缓冲区开销。ext4 可以使用 DAX。
- **排队自旋锁 (Queued spinlocks) (4.2)**：在争用下提供更好的性能，这些在 4.2 中成为默认的自旋锁内核实现。
- **TCP 无锁监听器 (4.4)**：TCP 监听器快速路径变为无锁，提高了性能。
- **cgroup v2 (4.5, 4.15)**：早期内核中曾有过统一的 cgroups 层级结构，在 4.5 中被认为是稳定的并公开，命名为 cgroup v2 [Heo 15]。cgroup v2 CPU 控制器在 4.15 中添加。
- **epoll 可扩展性 (4.5)**：对于多线程可扩展性，`epoll(7)` 避免了为每个事件唤醒所有在同一文件描述符上等待的线程，这曾导致“惊群”（thundering-herd）性能问题 [Corbet 15]。
- **KCM (4.6)**：内核连接多路复用器 (KCM) 提供了一个基于 TCP 的高效消息接口。
- **TCP NV (4.8)**：New Vegas (NV) 是一种适用于高带宽网络（运行速度在 10+ Gbps）的新型 TCP 拥塞控制算法。
- **XDP (4.8, 4.18)**：快速数据路径 (XDP) 是一种基于 BPF 的可编程快速路径，用于高性能网络 [Herbert 16]。4.18 中添加了可以绕过大部分网络栈的 `AF_XDP` 套接字地址族。
- **TCP BBR (4.9)**：瓶颈带宽和 RTT (BBR) 是一种 TCP 拥塞控制算法，在遭受丢包和 bufferbloat 的网络上提供改进的延迟和吞吐量 [Cardwell 16]。
- **硬件延迟追踪器 (4.9)**：一个 Ftrace 追踪器，可以检测由硬件和固件引起的系统延迟，包括系统管理中断 (SMIs)。
- **perf c2c (4.10)**：`cache-to-cache` (c2c) perf 子命令可以帮助识别 CPU 缓存性能问题，包括伪共享。
- **Intel CAT (4.10)**：支持 Intel 缓存分配技术 (CAT)，允许任务拥有专用的 CPU 缓存空间。这可以被容器用来帮助解决“吵闹邻居”问题。
- **多队列 I/O 调度器：BFQ, Kyber (4.12)**：预算公平队列 (BFQ) 多队列 I/O 调度器为交互式应用程序提供低延迟 I/O，特别是对于较慢的存储设备。BFQ 在 5.2 中得到了显著改进。Kyber I/O 调度器适用于快速的多队列设备 [Corbet 17]。
- **内核 TLS (4.13, 4.17)**：内核 TLS 的 Linux 版本 [Edge 15]。
- **MSG_ZEROCOPY (4.14)**：一个 `send(2)` 标志，用于避免应用程序和网络接口之间数据包字节的额外复制 [Linux 20c]。
- **PCID (4.14)**：Linux 增加了对进程上下文 ID (PCID) 的支持，这是一种处理器 MMU 特性，有助于避免在上下文切换时刷新 TLB。这降低了缓解 Meltdown 漏洞所需的内核页表隔离 (KPTI) 补丁的性能成本。见 3.4.3 节“KPTI (Meltdown)”。
- **PSI (4.20, 5.2)**：压力停顿信息 (PSI) 是一组新的指标，显示在 CPU、内存或 I/O 上停顿的时间。5.2 中增加了 PSI 阈值通知以支持 PSI 监控。
- **TCP EDT (4.20)**：TCP 栈切换到了早期离去时间 (EDT)：这使用时间轮调度程序发送数据包，提供更好的 CPU 效率和更小的队列 [Jacobson 18]。
- **多队列 I/O (5.0)**：多队列块 I/O 调度程序在 5.0 中成为默认设置，经典的调度程序被删除。
- **UDP GRO (5.0)**：UDP 通用接收卸载 (GRO) 通过允许驱动程序和网卡聚合数据包并向上传递栈来提高性能。
- **io_uring (5.1)**：一个通用的异步接口，用于应用程序和内核之间利用共享环形缓冲区的快速通信。主要用途包括快速磁盘和网络 I/O。
- **MADV_COLD, MADV_PAGEOUT (5.4)**：这些 `madvise(2)` 标志是对内核的提示，表示内存是需要的，但不是近期需要。`MADV_PAGEOUT` 也是一个内存可以立即回收的提示。这些对于内存受限的嵌入式 Linux 设备特别有用。
- **多路径 TCP (5.6)**：可以使用多个网络链路（例如 3G 和 WiFi）来提高单个 TCP 连接的性能和可靠性。
- **启动时追踪 (5.6)**：允许 Ftrace 追踪早期启动过程。（systemd 可以提供后期启动过程的时间信息：见 3.4.2 节“systemd”。）
- **热压力 (5.7)**：调度器考虑热节流（thermal throttling）以做出更好的放置决策。
- **perf 火焰图 (5.8)**：`perf(1)` 对火焰图可视化的支持。

这里未列出针对锁、驱动程序、VFS、文件系统、异步 I/O、内存分配器、NUMA、新处理器指令支持、GPU 以及性能工具 `perf(1)` 和 Ftrace 的许多细微性能改进。通过采用 systemd，系统启动时间也得到了改善。

接下来的章节将更详细地描述对性能重要的三个 Linux 主题：systemd、KPTI 和扩展 BPF。

### 3.4.2 systemd

systemd 是 Linux 常用的服务管理器，作为原始 UNIX init 系统的替代方案而开发。systemd 具有各种特性，包括感知依赖的服务启动和服务时间统计。

系统性能中的一项偶尔任务是调整系统的启动时间，而 systemd 时间统计可以显示在哪里进行调整。可以使用 `systemd-analyze(1)` 报告总的启动时间：
...
此输出显示系统在 9.663 秒内启动（在本例中达到了 `graphical.target`）。使用 `critical-chain` 子命令可以看到更多信息：
...
此输出显示了关键路径：导致延迟的步骤序列（在本例中为服务）。启动最慢的服务是 `systemd-networkd-wait-online.service`，耗时 1.86 秒。

还有其他有用的子命令：`blame` 显示最慢的初始化时间，`plot` 生成 SVG 图表。更多信息请参见 `systemd-analyze(1)` 的帮助手册。

### 3.4.3 KPTI (Meltdown)

2018 年添加到 Linux 4.14 中的[[内核页表隔离]]（KPTI）补丁是针对名为“Meltdown”（熔断）的 Intel 处理器漏洞的缓解措施。较旧的 Linux 内核版本曾有用于类似目的的 KAISER 补丁，其他内核也采用了缓解措施。虽然这些措施解决了安全问题，但由于上下文切换和系统调用时额外的 CPU 周期以及额外的 TLB 刷新，它们也降低了处理器性能。Linux 在同一版本中增加了[[进程上下文 ID]]（PCID）支持，如果处理器支持 `pcid`，则可以避免一些 TLB 刷新。

我评估了 KPTI 对 Netflix 云生产工作负载的性能影响在 0.1% 到 6% 之间，具体取决于工作负载的系统调用率（越高成本越大）[Gregg 18a]。进一步的调优将进一步降低成本：使用大页（huge pages）以便刷新的 TLB 更快地预热，以及使用追踪工具检查系统调用以识别减少其频率的方法。许多此类追踪工具是使用扩展 BPF 实现的。

### 3.4.4 扩展 BPF (Extended BPF)

BPF 代表伯克利包过滤器（Berkeley Packet Filter），这是一种最初开发于 1992 年的晦涩技术，用于提高数据包捕获工具的性能 [McCanne 92]。2013 年，Alexei Starovoitov 提出了对 BPF 的重大重写 [Starovoitov 13]，随后由他本人和 Daniel Borkmann 进一步开发，并于 2014 年包含在 Linux 内核中 [Borkmann 14b]。这使 BPF 变成了一个通用的执行引擎，可用于各种任务，包括网络、可观测性和安全性。

BPF 本身是一种灵活且高效的技术，由指令集、存储对象（映射 maps）和辅助函数组成。由于其虚拟指令集规范，它可以被认为是一个虚拟机。BPF 程序在内核态运行（如前面的图 3.2 所示），并配置为在事件发生时运行：套接字事件、tracepoints、USDT 探针、kprobes、uprobes 和 perf_events。这些如图 3.16 所示。

![BPF 组件](images/chapter-003/page0160_img001.jpeg)
**图 3.16 BPF 组件**

BPF 字节码必须首先通过验证器（verifier）进行安全检查，以确保 BPF 程序不会崩溃或损坏内核。它还可能使用 BPF 类型格式（BTF）系统来理解数据类型和结构。BPF 程序可以通过 perf 环形缓冲区（一种发出每事件数据的有效方式）或通过映射（适合统计数据）输出数据。

由于 BPF 为新一代高效、安全和先进的追踪工具提供了动力，它对于系统性能分析非常重要。它为现有的内核事件源（tracepoints、kprobes、uprobes 和 perf_events）提供了可编程性。例如，BPF 程序可以在 I/O 开始和结束时记录时间戳以测量其持续时间，并将其记录在自定义直方图中。本书包含许多使用 BCC 和 bpftrace 前端的基于 BPF 的程序。这些前端在第 15 章中涵盖。

## 3.5 其他主题 (Other Topics)

其他一些值得总结的内核和操作系统主题包括 PGO 内核、独核（Unikernels）、微内核、混合内核和分布式操作系统。

### 3.5.1 PGO 内核

[[配置文件引导优化]]（PGO），也称为[[反馈引导优化]]（FDO），使用 CPU 剖析信息来改进编译器的决策 [Yuan 14a]。这可以应用于内核构建，其过程为：

1. 在生产环境中，获取 CPU 剖析数据。
2. 基于该 CPU 剖析数据重新编译内核。
3. 在生产环境中部署新内核。

这创建了一个针对特定工作负载改进了性能的内核。诸如 JVM 之类的运行时会自动执行此操作，结合即时（JIT）编译，根据运行时性能重新编译 Java 方法。而创建 PGO 内核的过程则涉及手动步骤。

一个相关的编译优化是[[链接时间优化]]（LTO），其中整个二进制文件被一次性编译，以允许在整个程序中进行优化。Microsoft Windows 内核大量使用 LTO 和 PGO，通过 PGO 获得了 5% 到 20% 的提升 [Bearman 20]。Google 也使用 LTO 和 PGO 内核来提高性能 [Tolvanen 20]。

gcc 和 clang 编译器以及 Linux 内核都支持 PGO。内核 PGO 通常涉及运行一个专门插桩的内核来收集剖析数据。Google 发布了一个 AutoFDO 工具，绕过了对此类特殊内核的需求：AutoFDO 允许使用 `perf(1)` 从普通内核收集剖析数据，然后将其转换为编译器可以使用的正确格式 [Google 20a]。

关于构建具有 PGO 或 AutoFDO 的 Linux 内核，唯一的最新文档是来自 Microsoft [Bearman 20] 和 Google [Tolvanen 20] 在 2020 年 Linux Plumber’s Conference 上的两次演讲。[^15]

### 3.5.2 独核 (Unikernels)

[[独核]]（Unikernel）是一种单应用程序机器镜像，它将内核、库和应用程序软件结合在一起，通常可以在硬件虚拟机或裸机上的单个地址空间中运行。这具有潜在的性能和安全优势：指令文本更少意味着更高的 CPU 缓存命中率和更少的安全漏洞。这也产生了一个问题：可能没有 SSH、shell 或性能工具可供你登录和调试系统，也没有任何方式添加它们。

为了使独核在生产环境中进行性能调优，必须构建新的性能工具和指标来支持它们。作为概念验证，我构建了一个从 Xen dom0 运行的初级 CPU 剖析器，用于剖析 domU 独核访客，然后构建了 CPU 火焰图，只是为了证明这是可能的 [Gregg 16a]。

独核的例子包括 MirageOS [MirageOS 20]。

### 3.5.3 微内核与混合内核 (Microkernels and Hybrid Kernels)

本章的大部分内容讨论类 Unix 内核，也被称为[[宏内核]]（Monolithic kernels），其中所有管理设备的代码都作为单个大型内核程序一起运行。对于[[微内核]]（Microkernel）模型，内核软件被保持在最低限度。微内核支持内存管理、线程管理和进程间通信（IPC）等核心要素。文件系统、网络栈和驱动程序作为用户态软件实现，这使得这些用户态组件更容易被修改和替换。想象一下，不仅可以选择安装哪个数据库或 Web 服务器，还可以选择安装哪个网络栈。微内核也更具容错性：驱动程序中的崩溃不会导致整个内核崩溃。微内核的例子包括 QNX 和 Minix 3。

微内核的一个缺点是，执行 I/O 和其他功能需要额外的 IPC 步骤，从而降低了性能。解决此问题的一个方案是[[混合内核]]（Hybrid kernels），它结合了微内核和宏内核的优点。混合内核将性能关键型服务移回内核空间（使用直接函数调用而不是 IPC），就像宏内核一样，但保留了微内核的模块化设计和容容错性。混合内核的例子包括 Windows NT 内核和 Plan 9 内核。

### 3.5.4 分布式操作系统 (Distributed Operating Systems)

[[分布式操作系统]]在通过网络连接的一组独立计算机节点上运行单个操作系统实例。每个节点上通常使用微内核。分布式操作系统的例子包括来自贝尔实验室的 Plan 9 和 Inferno 操作系统。

虽然这是一个创新的设计，但该模型尚未得到广泛应用。Plan 9 和 Inferno 的共同创作者 Rob Pike 描述了造成这种情况的各种原因，包括 [Pike 00]：

> “在 1970 年代末和 1980 年代初有一种说法，认为 Unix 扼杀了操作系统研究，因为没有人会尝试其他任何东西。当时，我不相信。今天，我勉强接受这一说法可能是真的（尽管有微软的存在）。”

在云端，当今通用的计算节点扩展模型是在一组相同的操作系统实例之间进行负载均衡，这些实例可以根据负载进行扩展（见第 11 章“云计算”11.1.3 节“容量规划”）。

## 3.6 内核对比 (Kernel Comparisons)

哪个内核最快？这在一定程度上取决于操作系统配置、工作负载以及内核参与的程度。总的来说，我预计 Linux 将优于其他内核，因为它在性能改进、应用程序和驱动程序支持方面进行了大量工作，且应用广泛，拥有发现并报告性能问题的大型社区。自 1993 年以来由 TOP500 榜单追踪的全球前 500 强超级计算机在 2017 年全部采用了 Linux [TOP500 17]。也会有一些例外；例如，Netflix 在云端使用 Linux，而在其 CDN 上使用 FreeBSD。[^16]

内核性能通常使用[[微基准测试]]（Micro-benchmarks）进行比较，这很容易出错。此类基准测试可能会发现一个内核在某个特定的系统调用上快得多，但生产工作负载并不使用该系统调用。（或者它确实被使用了，但带有基准测试未测试的某些标志，而这些标志极大地影响了性能。）准确比较内核性能是高级性能工程师的任务——这项任务可能需要数周时间。见第 12 章“基准测试”12.3.2 节“主动基准测试”作为应遵循的方法论。

在本书的第一版中，我通过指出 Linux 没有成熟的动态追踪器来结束本节，如果没有它，你可能会错过巨大的性能提升。自第一版以来，我转到了全职 Linux 性能岗位，并参与开发了 Linux 缺失的动态追踪器：BCC 和 bpftrace（基于扩展 BPF）。这些在第 15 章和我的上一本书 [Gregg 19] 中有介绍。

3.4.1 节“Linux 内核开发”列出了第一版和本版之间发生的许多其他 Linux 性能开发，跨越了内核版本 3.1 到 5.8。早先未列出的一个主要开发是 OpenZFS 现在支持 Linux 作为其主要内核，为 Linux 提供了一个高性能且成熟的文件系统选项。

然而，随着所有这些 Linux 开发而来的是复杂性。Linux 上有如此多的性能特性和可调参数，以至于为每个工作负载配置和调整它们已变得非常吃力。我见过许多部署在未调优的情况下运行。在比较内核性能时请记住这一点：每个内核都经过调优了吗？本书后面的章节及其调优部分可以帮助你补救这一点。

## 3.7 练习 (Exercises)

1. 回答下列关于操作系统术语的问题：
   - 进程（process）、线程（thread）和任务（task）之间有什么区别？
   - 什么是[[模式切换]]（mode switch）和[[上下文切换]]（context switch）？
   - [[分页]]（paging）和[[进程交换]]（process swapping）之间有什么区别？
   - [[I/O 密集型]]（I/O-bound）和 [[CPU 密集型]]（CPU-bound）工作负载之间有什么区别？
2. 回答下列概念性问题：
   - 描述内核的作用。
   - 描述系统调用的作用。
...
操作系统及其内核是一个迷人且广泛的话题。本章仅总结了要点。除了本章提到的来源外，以下也是优秀的参考资料，适用于基于 Linux 的操作系统和其他系统：
...
[^14]: 开发目的是为了提高 Netflix FreeBSD Open Connect Appliances (OCAs) 的性能，这些设备构成了 Netflix 的 CDN。
[^15]: 有一段时间，最新的文档是 2014 年关于 Linux 3.13 的 [Yuan 14b]，这阻碍了在较新内核上的采用。
[^16]: FreeBSD 为 Netflix CDN 工作负载提供了更高的性能，特别是由于 Netflix OCA 团队进行的内核改进。这会定期进行测试，最近一次是在 2019 年，我对 Linux 5.0 与 FreeBSD 的生产对比进行了分析。
