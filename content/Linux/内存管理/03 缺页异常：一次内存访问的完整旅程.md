...
> [!note] 思考题
> 1. Major Page Fault（需要磁盘 IO）和 Minor Page Fault（只需内存操作）的性能差异有多大？在延迟敏感的应用中，如何通过 `mlock()` 或 `MAP_POPULATE` 避免运行时 Page Fault？`mlockall(MCL_CURRENT | MCL_FUTURE)` 的风险是什么？
> 2. Copy-on-Write（COW）是 fork() 的核心优化。Redis RDB 持久化 fork 后，如果父进程持续修改大量内存页，COW 可能导致内存使用翻倍。你如何通过 `info memory` 的 `mem_fragmentation_ratio` 监控这种情况？在什么内存使用模式下 COW 的代价最大？
> 3. Demand Paging 意味着 malloc 不立即获得物理页。`vm.overcommit_memory` 的三种模式对应用有什么影响？在容器环境中 overcommit 策略如何与 CGroups 内存限制配合？Java 应用启动时用 `-XX:+AlwaysPreTouch` 预触发所有页面有什么利弊？
