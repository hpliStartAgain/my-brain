...
> [!note] 思考题
> 1. Java 9 废弃了 `java.util.Observable` 和 `Observer`。Spring 的 `ApplicationEvent` + `@EventListener` 是更现代的观察者实现。同步事件发布（`ApplicationEventPublisher.publishEvent()`）意味着所有监听器在发布线程中依次执行——如果某个监听器执行缓慢，会阻塞发布者。`@Async` + `@EventListener` 能解决这个问题吗？异步事件在事务环境中有什么陷阱（如事务未提交时事件已被消费）？
> 2. 责任链模式在 Java Web 中随处可见——Servlet Filter、Spring Interceptor、Netty ChannelPipeline 都是责任链。Servlet Filter 的顺序由 `web.xml` 或 `@Order` 注解决定。如果两个 Filter 的 Order 相同，它们的执行顺序是确定的吗？在微服务网关中，如何保证 Filter 链的顺序在多实例部署时一致？
