...
const defaultOptions: ArticleToolsOptions = {
  showProgress: true,
  showCopy: true,
  copyLabel: "复制正文",
}
 ...
 */
const ArticleTools: QuartzComponentConstructor<Partial<ArticleToolsOptions>> = (userOpts) => {
  const opts = { ...defaultOptions, ...userOpts }

  const ArticleToolsComponent: QuartzComponent = ({ displayClass }: QuartzComponentProps) => {
    return (
      <>
        {opts.showProgress && (
          <div class="reading-progress" data-reading-progress aria-hidden="true">
            <div class="reading-progress-bar" data-reading-progress-bar />
          </div>
        )}
        {opts.showCopy && (
          <div class={displayClass ? `${displayClass} article-tools` : "article-tools"}>
            <button type="button" class="copy-article-btn" data-copy-article aria-label={opts.copyLabel}>
              <svg
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
              ...
              <span data-copy-label>{opts.copyLabel}</span>
            </button>
          </div>
        )}
      </>
